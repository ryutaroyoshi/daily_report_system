...
public abstract class ActionBase {
    ...
    protected void invoke() throws ServletException,IOException{
        ...
        try {
            ...
            //（例:action =Employee command=show の場合 EmployeeActionクラスのshow()メソッドを実行）
            ...
            //commandの値が不正で実行できない場合のエラー画面を呼び出し
    ...
    /*
     * 指定されたjspの呼び出しを行う
     * @param taget 遷移先jsp画面のファイル名（拡張子を含まない）
     * @throws ServletException
     * @throws IOException
     */
    ...
    /*
     * アプリケーションスコープから指定したパラメータの値を取得し、返却する
     * @param key パラメータ名
     * @return パラメータの値
     */
    @SuppressWarnings("unchecked")
